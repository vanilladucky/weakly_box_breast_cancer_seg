    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        while fg_sample is not None and bg_sample is not None:
            ...
            if np.unique(cor_seg.data.cpu().numpy()).size == 1 and np.unique(cor_seg.data.cpu().numpy())[0] == 0:
                ...
                l_ce = CE(outs, segs)
                l_proj = 0.05 * torch.abs(LogBarrier.penalty(z0) + LogBarrier.penalty(z1) + LogBarrier.penalty(z2)) # Might need clamping to ensure no 'rewarding' for overshooting (can try torch.abs())
