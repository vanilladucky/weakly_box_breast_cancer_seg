...
class PermutohedralLattice(torch.autograd.Function):
    ...
    @staticmethod
    def prepare(feat):
        B, n_ch, n_voxels = feat.size()  # batch_size, coordinate channel(3) + img channel(1), number of voxels
        ...
        for scit in range(n_ch + 1):
            ...
            _ = HT_opp.insert(table, n_entries, loc[scit].type(torch.cuda.IntTensor),
                              loc_hash[scit].type(torch.cuda.IntTensor))
            del (_)
